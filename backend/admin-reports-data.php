...
// Monthly product revenue trend (last 6 months, completed orders only)
...
if ($result = mysqli_query($conn, $sql)) {
    while ($row = mysqli_fetch_assoc($result)) {
        $monthly_product_revenue[] = $row;
    }
}

// Top 5 products by quantity sold (completed orders only)
$top_products = [];
$sql = "SELECT p.product_name,
               SUM(poi.quantity) AS total_sold,
               SUM(poi.quantity * poi.price_at_time) AS total_revenue
        FROM product_order_items poi
        JOIN products p ON poi.product_id = p.id
        JOIN product_orders po ON poi.order_id = po.id
...
// Overall product revenue total (all-time, completed orders only)
$grand_total_product_revenue = 0;
$sql = "SELECT SUM(total_amount) AS total FROM product_orders WHERE status = 'completed'";
if ($result = mysqli_query($conn, $sql)) {
    $grand_total_product_revenue = mysqli_fetch_assoc($result)['total'] ?? 0;
}