<?php
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

if (!isset($_SESSION['user_id'])) {
    header("Location: login.php");
    exit();
}

if ($_SESSION['user_role'] !== 'admin') {
    header("Location: index.php");
    exit();
}

$admin_name = $_SESSION['user_name'] ?? 'Admin';

require_once __DIR__ . '/../backend/config.php';
global $conn;

require_once __DIR__ . '/../backend/admin-staff-performance-data.php';

// Calculate max values safely for relative bar chart scaling
$max_appointments = 0;
$max_revenue = 0;

if (!empty($staff_performance) && is_array($staff_performance)) {
    foreach ($staff_performance as $s) {
        $appts = $s['total_appointments'] ?? 0;
        $rev = $s['total_revenue'] ?? 0;
        
        if ($appts > $max_appointments) $max_appointments = $appts;
        if ($rev > $max_revenue) $max_revenue = $rev;
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Salon You - Staff Performance</title>
    <!-- FontAwesome Icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <!-- External CSS Link -->
    <link rel="stylesheet" href="frontend-css/staff-perfomance.css">
</head>
<body>
<div class="admin-wrapper">
    <!-- Sidebar Navigation -->
    <div class="sidebar">
        <div class="sidebar-header">
            <h2>Salon You</h2>
            <p>Admin Control Panel</p>
        </div>
        <ul class="sidebar-menu">
            <li><a href="admin-dashboard.php"><i class="fas fa-chart-line"></i><span> Dashboard</span></a></li>
            <li class="active"><a href="admin-staff.php"><i class="fas fa-users"></i><span> Staff Management</span></a></li>
            <li><a href="admin-customers.php"><i class="fas fa-user-friends"></i><span> Customer Management</span></a></li>
            <li><a href="admin-appointments.php"><i class="fas fa-calendar-check"></i><span> Appointments</span></a></li>
            <li><a href="admin-queue.php"><i class="fas fa-list-ol"></i><span> Today's Queue</span></a></li>
            <li><a href="admin-services.php"><i class="fas fa-cut"></i><span> Services</span></a></li>
            <li><a href="admin-products.php"><i class="fas fa-pump-soap"></i><span> Product</span></a></li>
            <li><a href="admin-product-orders.php"><i class="fas fa-shopping-basket"></i><span> Product Orders</span></a></li>
            <li><a href="admin-inventory.php"><i class="fas fa-box"></i><span> Inventory</span></a></li>
            <li><a href="admin-billing.php"><i class="fas fa-money-bill"></i><span> Billing & Payments</span></a></li>
            <li><a href="admin-reports.php"><i class="fas fa-file-invoice-dollar"></i><span> Reports</span></a></li>
            <li><a href="admin-closed-dates.php"><i class="fas fa-calendar-times"></i><span> Closed Dates</span></a></li>
            <li><a href="admin-notifications.php"><i class="fas fa-bell"></i><span> Notifications</span></a></li>
            <li><a href="../backend/logout.php"><i class="fas fa-sign-out-alt"></i><span> Logout</span></a></li>
        </ul>
    </div>

    <!-- Main Workspace -->
    <div class="main-content">
        <div class="topbar">
            <h3>Welcome, <?php echo htmlspecialchars($admin_name); ?></h3>
        </div>

        <div class="page-header">
            <h2>Staff Performance & Analytics</h2>
        </div>

        <!-- Summary KPI Cards -->
        <div class="cards-row">
            <div class="card purple">
                <h4>Total Active Staff</h4>
                <p class="card-number"><?php echo $total_staff ?? 0; ?></p>
            </div>
            <div class="card green">
                <h4>Total Staff Generated Revenue</h4>
                <p class="card-number">Rs. <?php echo number_format($staff_revenue_total ?? 0, 2); ?></p>
            </div>
        </div>

        <!-- Detailed Staff Performance List -->
        <div class="panel">
            <h4>Individual Staff Performance Metrics</h4>
            <?php if (empty($staff_performance)): ?>
                <p style="color: var(--text-muted); font-size: 14px;">No staff records available.</p>
            <?php else: ?>
                <?php foreach ($staff_performance as $staff): 
                    $total_appts = $staff['total_appointments'] ?? 0;
                    $completed_appts = $staff['completed_appointments'] ?? 0;
                    $revenue = $staff['total_revenue'] ?? 0;
                    $completion_rate = $total_appts > 0 ? round(($completed_appts / $total_appts) * 100) : 0;
                ?>
                    <div class="staff-card">
                        <div class="staff-card-header">
                            <div>
                                <div class="staff-name"><?php echo htmlspecialchars($staff['name'] ?? 'Unknown'); ?></div>
                                <div class="staff-contact"><?php echo htmlspecialchars($staff['email'] ?? 'N/A'); ?> | <?php echo htmlspecialchars($staff['phone'] ?? 'N/A'); ?></div>
                            </div>
                            <?php if (!empty($staff['avg_rating'])): ?>
                                <div class="rating-badge">
                                    <i class="fas fa-star"></i> <?php echo number_format($staff['avg_rating'], 1); ?> / 5
                                </div>
                            <?php else: ?>
                                <div class="rating-badge no-rating">No Ratings</div>
                            <?php endif; ?>
                        </div>

                        <!-- Numeric Metrics -->
                        <div class="staff-metrics">
                            <div class="metric-item">
                                <div class="metric-value"><?php echo $total_appts; ?></div>
                                <div class="metric-label">Total Appointments</div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-value"><?php echo $completed_appts; ?></div>
                                <div class="metric-label">Completed</div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-value">Rs. <?php echo number_format($revenue, 0); ?></div>
                                <div class="metric-label">Generated Revenue</div>
                            </div>
                        </div>

                        <!-- Completion Percentage Bar -->
                        <div class="bar-group">
                            <div class="bar-row">
                                <div class="bar-label">
                                    <span>Completion Rate</span>
                                    <span><?php echo $completion_rate; ?>%</span>
                                </div>
                                <div class="bar-track">
                                    <div class="bar-fill success" style="width: <?php echo $completion_rate; ?>%;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
    </div>
</div>
</body>
</html>